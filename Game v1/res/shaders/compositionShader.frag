...
vec4 blur(){
	...
	sum += texture2D(effectColorTex, vec2(varyingTexCoord0.x, clamp(varyingTexCoord0.y - 2.0 * dist, min, max))) * 0.05;
	sum += texture2D(effectColorTex, vec2(varyingTexCoord0.x, clamp(varyingTexCoord0.y - dist, min, max))) * 0.2;
